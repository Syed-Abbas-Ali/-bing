...
// export function responseBuilder (serviceResponse: IServiceResponse, res: Response,req?: Request, next?: NextFunction): void {
...
export function responseBuilder(
  ...
  try {
    ...
    if (serviceResponse.errors && serviceResponse.errors.length !== 0) {
      ...
      res.status(serviceResponse.statusCode ?? (req && req.method === 'POST' ? 201 : 200)).send(serviceResponse);
    }
  } catch (error) {
    log.error('ERROR occurred in helper.response_builder.responseBuilder()', error);
    if (next) {
      ...
      // Handle the error in an appropriate way if next is not provided
